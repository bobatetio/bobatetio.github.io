...
(function () {
  ...
  /* The Common Questions block. The accordion above it drives .ukFaq_* markup,
     which this section does not use: it is .faq-item / .faq-item__header /
     .faq-item__answer, and its CSS opens on an .is-open class that nothing ever
     set. So the panel was inert. This wires that markup, and makes the headers
     real controls rather than plain divs. */
  function faqItems() {
    var items = [].slice.call(document.querySelectorAll('.faq-item'));
    if (!items.length) return;
    items.forEach(function (it, i) {
      var head = it.querySelector('.faq-item__header');
      var answer = it.querySelector('.faq-item__answer');
      if (!head || !answer || head.dataset.faq === '1') return;
      head.dataset.faq = '1';
      if (!answer.id) answer.id = 'faq-a-' + i;
      head.setAttribute('role', 'button');
      head.setAttribute('tabindex', '0');
      head.setAttribute('aria-controls', answer.id);
      ...
      function toggle() {
        ...
        items.forEach(function (o) {
          ...
          var oh = o.querySelector('.faq-item__header');
