...
(function () {
  ...
  var STATIC = !!document.querySelector('[data-surfer-static]');
  ...
  function init() {
    ...
    function frame(now) {
      ...
      /* A page can pin the row to its opening pose with data-surfer-static: the
         cards then sit exactly where the choreography's first frame puts them —
         same rail, same depth scaling, same spacing — and never advance.

         Pinning the ride is only half of keeping the row in the hero. The scene
         is a fixed full-viewport layer, so a pinned pose is nailed to the SCREEN
         rather than to the page: the row hangs where it is while the sections
         scroll up underneath it. Offsetting the layer by the scroll distance is
         what makes fixed behave as though it were anchored in the hero, so the
         row rises and leaves with it. */
      if (STATIC) surfer.style.transform = 'translate3d(0,' + (-scroll) + 'px,0)';
      var target = (reduce || STATIC) ? 0 : (isNaN(forced) ? scroll / PER_CARD : forced);
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        if (v) {
          ...
          // over the video. That stays on hover and on the docked card, the
          // way it was, so autoplay never covers the footage.
          card.classList.toggle('is-playing', (i === 0 && docked) || hoverNow);
        }
